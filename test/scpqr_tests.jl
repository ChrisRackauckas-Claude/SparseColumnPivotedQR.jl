...
# Steady-state allocation of one `scpqr_refactor!(adaptive_dense=true)` and one
...
function _refactor_alloc(F, A)
    return @allocated scpqr_refactor!(F, A; adaptive_dense = true)
end
...
function _dense_zeroalloc_case(::Type{T}, A64, n) where {T}
    ...
    b = ones(T, size(A, 1))
    x = zeros(T, n)
    sym = scpqr_analyze(A; ordering = :amd)
    F = scpqr_factor(A, sym; adaptive_dense = true)
    scpqr_refactor!(F, A; adaptive_dense = true); ldiv!(x, F, b)   # warm
    scpqr_refactor!(F, A; adaptive_dense = true); ldiv!(x, F, b)
    ra = _refactor_alloc(F, A)
    la = _ldiv_alloc(x, F, b)
    x1 = zeros(T, n); scpqr_refactor!(F, A; adaptive_dense = true); ldiv!(x1, F, b)
    x2 = zeros(T, n); scpqr_refactor!(F, A; adaptive_dense = true); ldiv!(x2, F, b)
...
@testset "SparseColumnPivotedQR" begin
    ...
    @testset "Issue #23: rank-deficient LS reaches minimum residual" begin
        ...
        for ordering in (:amd, :natural)
            F = scpqr_factor(Acsr, scpqr_analyze(Acsr; ordering = ordering))
            x = F \ b
    ...
    @testset "Issue #23: square rank-deficient interleaved dependence" begin
        ...
        F = scpqr_factor(Acsr, scpqr_analyze(Acsr; ordering = :amd))
        x = F \ b
    ...
    @testset "analyze + factor split" begin
        ...
        @test size(sym) == (n, n)
        F = scpqr_factor(Acsr, sym)
        x = F \ b
        @test rank(F) == n
        @test norm(A * x - b) / norm(b) < 1.0e-10

        # scpqr matches scpqr_factor(scpqr_analyze)
        F2 = scpqr(Acsr; ordering = :natural)
        x2 = F2 \ b
        @test rank(F2) == n
    ...
    @testset "AMD ordering" begin
        ...
        sym_nat = scpqr_analyze(Acsr; ordering = :natural)
        sym_amd = scpqr_analyze(Acsr; ordering = :amd)
        sym_col = scpqr_analyze(Acsr; ordering = :colamd)
        ...
        F_nat = scpqr_factor(Acsr, sym_nat); x_nat = F_nat \ b
        F_amd = scpqr_factor(Acsr, sym_amd); x_amd = F_amd \ b
    ...
    @testset "default ordering resolves to :amd when AMD is loaded" begin
        ...
        sym_default = scpqr_analyze(Acsr)            # implicit :default
        sym_amd = scpqr_analyze(Acsr; ordering = :amd)
        @test sym_default.ordering === :amd
        @test sym_default.q == sym_amd.q

        F_default = scpqr(Acsr)
        F_amd = scpqr(Acsr; ordering = :amd)
    ...
    @testset "default ordering on bundled 199x199 matrices" begin
        ...
        for f in files
            ...
            A = eval(Meta.parse(strip(lines[1])))
            Acsr = sparse(A)
            sym_default = scpqr_analyze(Acsr)
            sym_amd = scpqr_analyze(Acsr; ordering = :amd)
            # :default == :amd here.
            @test sym_default.q == sym_amd.q
            ...
            # dense-fill matrices.
            F_nat = scpqr(Acsr; ordering = :natural)
            F_amd = scpqr(Acsr; ordering = :amd)
            @test length(F_amd.R_nzval) < length(F_nat.R_nzval)
        end
    end

    @testset "ordering propagated through scpqr" begin
        Random.seed!(14)
        n = 20
    ...
    @testset "adaptive ordering picks the shallower etree" begin
        ...
        for f in files
            ...
            b = eval(Meta.parse(strip(lines[2])))
            Acsr = sparse(A)
            sym_adapt = scpqr_analyze(Acsr; ordering = :adaptive)
            @test sym_adapt.ordering === :amd
            F = scpqr_factor(Acsr, sym_adapt)
            if all(isfinite, b)
                x = F \ b
                @test all(isfinite, x)
            end
    ...
    @testset "drop_tol prunes V columns and stays within tolerance" begin
        ...
        F0 = scpqr(Acsr; ordering = :natural)
        F1 = scpqr(Acsr; ordering = :natural, drop_tol = 1.0e-8)
        ...
        F2 = scpqr(Acsr; ordering = :natural, drop_tol = 0)
        @test F2.V_nzval == F0.V_nzval
        @test F2.beta == F0.beta
    end

    @testset "Workspace pool: scpqr_refactor! is zero-alloc steady-state" begin
        Random.seed!(21)
        n = 30
        Asp = sprand(Float64, n, n, 0.2)
        rows, cols, _ = findnz(Asp)
        ...
        nbytes = @allocated scpqr_refactor!(F, A2)
        @test nbytes == 0
        nbytes = @allocated scpqr_refactor!(F, A1)
        @test nbytes == 0
    ...
    @testset "Dense-tail apply matches the sparse reference solve" begin
        ...
        for f in files
            ...
            Acsr = sparse(A)
            x_sparse = scpqr(Acsr; ordering = :amd) \ b
            Fd = scpqr(Acsr; ordering = :amd, adaptive_dense = true)
            x_dense = Fd \ b
    ...
    @testset "Workspace pool: scpqr_refactor! mutates F in place" begin
        # The mutation-in-place semantic of scpqr_refactor! means the returned
    ...
    @testset "Workspace pool: handles different element types" begin
        ...
        sym = scpqr_analyze(Ar_csr; ordering = :natural)
        Fr = scpqr_factor(Ar_csr, sym)
    ...
    @testset "Symbolic exact R column counts" begin
        # The `rcount` field of SparseColumnPivotedQRSymbolic should equal the per-column nnz
        ...
        for (m, n, p) in [(20, 15, 0.3), (30, 20, 0.2), (50, 50, 0.15)]
            ...
            Acsr = build_csr(Matrix(A))
            for ordering in (:natural, :amd)
                sym = scpqr_analyze(Acsr; ordering = ordering)
                F = scpqr_factor(Acsr, sym)
    ...
    @testset "Adaptive dense fallback: square full-rank dense" begin
        ...
        Acsr = build_csr(Adense)
        b = randn(n)
        F = scpqr(Acsr; adaptive_dense = true, dense_threshold = 0.2)
        @test F.k_dense > 0
        @test F.k_dense < n
        @test rank(F) == n
        x = F \ b
    ...
    @testset "Adaptive dense fallback: rank-deficient block" begin
        ...
        F = scpqr(Acsr; tol = 1.0e-10, adaptive_dense = true, dense_threshold = 0.2)
        @test F.k_dense > 0
        @test rank(F) <= n - 2
        x = F \ b
        @test all(isfinite, x)
    ...
    @testset "Adaptive dense fallback: sparse never triggers" begin
        ...
        Acsr = build_csr(Matrix(A))
        b = randn(n)
        F = scpqr(Acsr; adaptive_dense = true, dense_threshold = 0.4)
        x = F \ b
        @test norm(A * x - b) / norm(b) < 1.0e-10
        # Compare to non-adaptive path
        F_nat = scpqr(Acsr; adaptive_dense = false)
        x_nat = F_nat \ b
    ...
    @testset "Adaptive dense fallback: refactor!" begin
        ...
        F1 = scpqr(Acsr1; adaptive_dense = true, dense_threshold = 0.2)
        x1 = F1 \ b
        @test norm(A1 * x1 - b) / norm(b) < 1.0e-10

        F2 = scpqr_refactor!(F1, Acsr2; adaptive_dense = true, dense_threshold = 0.2)
        x2 = F2 \ b
        @test norm(A2 * x2 - b) / norm(b) < 1.0e-10

        # Compare against fresh factor.
        F2_fresh = scpqr(Acsr2; adaptive_dense = true, dense_threshold = 0.2)
        x2_fresh = F2_fresh \ b
        @test norm(x2 - x2_fresh) / max(norm(x2_fresh), 1.0) < 1.0e-10
    end
    ...
    @testset "Adaptive dense fallback: matches non-adaptive on user matrices" begin
        ...
        if isdir(dir)
            ...
            for f in files
                ...
                b = eval(Meta.parse(strip(lines[2])))
                Acsr = sparse(A)
                F = scpqr(Acsr; adaptive_dense = true)
                Fspqr = qr(A)
                xspqr = Fspqr \ b
                x = F \ b
                if all(isfinite, b)
    ...
    @testset "Generic number types" begin
        ...
        @testset "ForwardDiff.Dual solve matches primal" begin
            ...
            xprimal = scpqr(makeA(p0)) \ b

            solve(p) = scpqr(makeA(p)) \ b
        ...
        @testset "adaptive_dense falls back for non-BLAS eltype" begin
            ...
            b = randn(n)
            p0 = [1.1, 0.3]
            solve(p) = scpqr(makeA(p); adaptive_dense = true) \ b
            xprimal = scpqr(makeA(p0)) \ b
            xdual = solve(ForwardDiff.Dual.(p0, (1.0, 0.0)))
            @test ForwardDiff.value.(xdual) ≈ xprimal rtol = 1.0e-10
            Fdual = scpqr(makeA(ForwardDiff.Dual.(p0, (1.0, 0.0))); adaptive_dense = true)
    ...
    @testset "adjoint / transpose solve" begin
        ...
        for T in (Float64, ComplexF64)
            ...
            A = build_csr(Adense)
            F = scpqr(A; ordering = :natural, adaptive_dense = true, dense_threshold = 0.1)
            @test F.k_dense > 0
            M = Matrix(Adense)
            b = rand(T, 30)
            x = adjoint(F) \ b
